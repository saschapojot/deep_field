...
if (len(sys.argv)!=7):
    ...
    print("example: python train_qt_dqnn.py num_epochs decrease_over decrease_rate step_num_after_S1 lin_increase_factor N")
    exit(argErrCode)
...
size_lin=int(lin_increase_factor*N*N)
print("size_lin:",size_lin)
model=DSNN_qt(
...
nonlinear_conv_linear_out_features1=size_lin,
nonlinear_conv_linear_out_features2=size_lin,
no_conv_lin_layer1_in_dim=size_lin,
no_conv_lin_layer1_out_dim=size_lin,
no_conv_lin_layer2_out_dim=size_lin
).to(device)
...
lin_increase_factorStr=format_using_decimal(lin_increase_factor)

out_model_dir=f"./out_model_data/N{N}/C{C}/layer{step_num_after_S1}/lin_increase_factor{lin_increase_factorStr}/"
Path(out_model_dir).mkdir(exist_ok=True,parents=True)
suffix_str=f"_over{decrease_overStr}_rate{decrease_rateStr}_epoch{num_epochs}_num_samples{num_samples_in}_lin_increase_factor{lin_increase_factorStr}"
