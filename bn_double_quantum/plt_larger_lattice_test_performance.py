...
def model_custom_err_vs_N(x,a,b,c):
    return a*x**b+c
N_vec_to_fit=N_vec[1:]
initial_guess = [1, -1, 1]
params,covariance=curve_fit(model_custom_err_vs_N,N_vec_to_fit,custom_err_vec[1:],initial_guess)
a,b,c=params
print(f"a={a},b={b},c={c}")
# print(f"covariance={covariance}")

plt_fit_data_num=100
plt_custom_err_N_vec=np.linspace(np.min(N_vec[1:]),np.max(N_vec[1:]),plt_fit_data_num)
...
plt.yticks([0.12,0.06, 0.02, 0.01], labels=["0.12","0.06", "0.02", "0.01"], fontsize=yTickSize)
