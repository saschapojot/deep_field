...
in_model_dir=f"./out_model_data/N{N}/upscale{upscale}/layer{step_num_after_S1}/"
...
model = dsnn_qt(
    ...
    phi0_out_channels=1,
    T_out_channels=1,
    nonlinear_conv1_out_channels=1,
    nonlinear_conv2_out_channels=1,
    final_out_channels=1,
    filter_size=filter_size,
stepsAfterInit=step_num_after_S1,upsample=use_up,upsample_scale=upscale
).to(device)
...
out_content=f"MSE_loss={format_using_decimal(test_loss)}, std_loss={format_using_decimal(std_loss)}  N={N}, upscale={upscale}, layer={step_num_after_S1}, decrease_over={decrease_overStr}, decrease_rate={decrease_rateStr}, num_epochs={num_epochs}"
