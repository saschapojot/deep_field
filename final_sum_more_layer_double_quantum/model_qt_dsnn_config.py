...
class CollapseChannels(nn.Module):
    def __init__(self, input_channels, output_channels, kernel_size,stride=1):
        """
        Collapses input channels to a single output channel for each sample in a batch.
        ...
        """
        super(CollapseChannels, self).__init__()
    ...
    def forward(self, x):
        ...
        Returns:
            torch.Tensor: Output tensor of shape (batch_size, output_channels, H, W).
        """
        return self.conv(x)
...
class dsnn_qt(nn.Module):
    def __init__(self, input_channels,
        ...
        self.T_layers_after_init=nn.ModuleList([
            TLayer(out_channels=T_out_channels, kernel_size=filter_size,padding=paddingNum) for _ in range(0,stepsAfterInit)
        ])
        ...
        #     padding=paddingNum
        # )
        self.collapse_channels = CollapseChannels(
            input_channels=nonlinear_conv2_out_channels,  # Matches the number of channels in S_n
            output_channels=final_out_channels,  # Collapse to a single channel
            kernel_size=filter_size,  # Use 1x1 convolution for channel reduction
            stride=1
        )
    ...
    def forward(self, x, Sn):
        ...
        final_output = self.collapse_channels(Sn)
